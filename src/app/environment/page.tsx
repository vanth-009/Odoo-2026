...
import EnvironmentDashboard from '@/environment/components/Dashboard';
import { AlertCircle, Database, RefreshCw } from 'lucide-react';

// Force dynamic rendering since we are doing database calls
export const dynamic = 'force-dynamic';

export default async function EnvironmentalDashboardPage() {
  try {
    // Retrieve initial dashboard metrics via direct database call
    const initialData = await getDashboardMetrics();

    return <EnvironmentDashboard initialData={initialData} />;
  } catch (error: any) {
    console.error('Failed to load dashboard metrics on server:', error);
    
    // Return a graceful error setup instruction page
    return (
        ...
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-center space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              The EcoSphere ESG Dashboard needs to connect to the MySQL database. 
              Please verify that your database URL in `.env` is correct and you have run migrations.
            </p>
          </div>
          ...
          <div className="p-3.5 bg-zinc-950 border border-zinc-850 rounded-xl text-left space-y-2 font-mono text-[10px] text-zinc-400">
            ...
            <div>
              ...
              <p className="text-zinc-500 mt-0.5">Check the DATABASE_URL environment variable in `.env` file.</p>
            </div>
            <div>
              <span className="text-emerald-400">2. Run migrations & seed data:</span>
              <p className="text-zinc-500 mt-0.5">Execute: <code className="text-zinc-300">npx prisma db push</code> then <code className="text-zinc-300">npx prisma db seed</code></p>
            </div>
          </div>
