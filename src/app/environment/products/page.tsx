...
import ProductsManager from '@/environment/components/ProductsManager';
import { AlertCircle, Database, RefreshCw } from 'lucide-react';
import { ProductESGProfile, EmissionFactor } from '@/environment/types';

// Force dynamic rendering since we are doing database queries
export const dynamic = 'force-dynamic';

export default async function ProductESGProfilesPage() {
  try {
    ...
    // 2. Fetch active emission factors for selection dropdowns
    ...
    // Type casting
    const typedProducts: ProductESGProfile[] = products.map(p => ({
      ...p,
      status: p.status as 'ACTIVE' | 'ARCHIVED',
      preferredEmissionFactor: p.preferredEmissionFactor ? {
        ...p.preferredEmissionFactor,
        status: p.preferredEmissionFactor.status as 'ACTIVE' | 'ARCHIVED'
      } : null
    }));

    const typedFactors: EmissionFactor[] = activeFactors.map(f => ({
      ...f,
      status: f.status as 'ACTIVE' | 'ARCHIVED'
    }));

    return <ProductsManager initialProducts={typedProducts} activeFactors={typedFactors} />;
  } catch (error: any) {
    console.error('Failed to load product ESG profiles on server:', error);
        ...
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-center space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              The EcoSphere ESG system needs to connect to the MySQL database to retrieve product profiles.
            </p>
          </div>
