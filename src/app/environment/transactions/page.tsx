...
import TransactionsManager from '@/environment/components/TransactionsManager';
import { AlertCircle, Database, RefreshCw } from 'lucide-react';
import { Department, ProductESGProfile, EmissionFactor } from '@/environment/types';
...
export default async function CarbonTransactionsPage() {
  try {
    ...
    const activeProducts = await prisma.productESGProfile.findMany({
      where: { status: 'ACTIVE' },
      orderBy: { name: 'asc' }
    });

    // 3. Fetch active emission factors
    ...
    const typedDepts: Department[] = departments.map(d => ({
      ...d,
      currentEmissions: 0, // Placeholder as required
      status: d.status as 'On Track' | 'Warning' | 'Critical',
      trend: d.trend as 'up' | 'down'
    }));

    const typedProducts: ProductESGProfile[] = activeProducts.map(p => ({
      ...p,
      status: p.status as 'ACTIVE' | 'ARCHIVED',
      preferredEmissionFactor: null // Optional
    }));
    ...
    return (
      <TransactionsManager 
        ...
        activeProducts={typedProducts} 
        activeFactors={typedFactors} 
      />
    );
  } catch (error: any) {
    console.error('Failed to load carbon transactions page dependencies:', error);
        ...
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-center space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              The EcoSphere carbon ledger needs to connect to the MySQL database to query transactions and reference entities.
            </p>
          </div>
