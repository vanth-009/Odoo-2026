...
import FactorsManager from '@/environment/components/FactorsManager';
...
export default async function EmissionFactorsPage() {
  try {
    ...
    const typedFactors = factors.map(f => ({
      ...f,
      status: f.status as 'ACTIVE' | 'ARCHIVED'
    }));

    return <FactorsManager initialFactors={typedFactors} />;
  } catch (error: any) {
    console.error('Failed to load emission factors on server:', error);
        ...
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-center space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              The EcoSphere ESG system needs to connect to the MySQL database to retrieve emission factors.
            </p>
          </div>
